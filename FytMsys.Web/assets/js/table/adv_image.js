$(function () {
    ...
    $("#Insert").click(function () {
        if (classId == 0) {
            dig.alertError("提示", "请选择添加广告的版位，点击左侧！");
            return;
        }
        dig.addModel("添加/编辑广告", url + "ImageModfiy/0?classId=" + classId, 850, 520, function () {
            tb.initTable(1);
            dig.remove();
        });
    });
    $("#Modfiy").click(function () {
        if (classId == 0) {
            dig.alertError("提示", "请选择编辑广告的版位，点击左侧！");
            return;
        }
        var vals = '', lsum = 0;
        ...
        dig.addModel("添加/编辑广告", url + "ImageModfiy/" + vals + "?classId=" + classId, 850, 520, function () {
            tb.initTable(1);
        });
    });
});
var classId = $("#classId").val();
var url = "/FytAdmin/AdvManage/", collSum = 9, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        ...
        $.ajax({
            ...
            data: {
                pageSize: pageSize, pageIndex: pageIndex, key: escape($("#txtKey").val()), classId: classId
            },
                ...
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        $("#tables tr").dblclick(function () {
                            ...
                            dig.addModel("添加/编辑广告", url + "ImageModfiy/" + eId + "?classId=" + classId, 850, 520, function () {
                                tb.initTable(1);
                            });
                        });
                    }
    ...
    deletes: function (id) {
        ...
        dig.confim("删除确认", msg, function () {
            $.post(url + "ImageDeleteBy", { id: vals }, function (res) {
