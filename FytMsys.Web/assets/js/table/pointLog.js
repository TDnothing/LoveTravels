$(function () {
    ...
    $("#Insert").click(function () {
        dig.addModel("添加/编辑KeyTags", url + "Modfiy/0", 650, 380, function () {
            tb.initTable(1);
            dig.remove();
        });
    });
    $("#Modfiy").click(function () {
        ...
        dig.addModel("添加/编辑KeyTags", url + "Modfiy/" + vals, 650, 380, function () {
            tb.initTable(1);
        });
    });
});

var url = "/FytAdmin/PointLog/", collSum = 8, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        ...
        $.ajax({
            ...
            data: {
                pageSize: pageSize, pageIndex: pageIndex, key: escape($("#txtKey").val()), userId: $("#userId").val(),
                Option: $("#Option").val()
            },
            success: function (res) {
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        //    dig.addModel("添加/编辑KeyTags", url + "Modfiy/" + eId, 650, 380, function () {
                        //        tb.initTable(1);
                        //    });
                        //});
                    }
