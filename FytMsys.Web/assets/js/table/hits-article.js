$(function () {
    ...
    $("#Modfiy").click(function () {
        ...
        window.location.href = url + "ArticleModfiy/" + vals + "?ClassId=" + $("#ClassId").val();
    });
});

var url = "/FytAdmin/Content/", collSum = 7, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        var pageSize = $("#sel_page").val(), maxi = 0;
        $.ajax({
            type: "post",
            url: url + "HitsIndexData",
    ...
    emptyRecyc: function () {
        ...
        dig.confim("清空确认", msg, function () {
            $.post(url + "emptyRecyc", { id: vals }, function (res) {
                if (res.Status == "y") tb.initTable(1);
                else dig.alertError("消息", res.Msg);
            }, "json");
        });
