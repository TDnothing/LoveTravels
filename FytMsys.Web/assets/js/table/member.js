$(function () {
    ...
    $("#Insert").click(function () {
        window.location.href = url + "Modfiy/0";
    });
    $("#Modfiy").click(function () {
        ...
        window.location.href = url + "Modfiy/" + vals;
    });
});
...
var url = "/FytAdmin/Member/", collSum = 8, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
                ...
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        $("#tables tr").dblclick(function () {
                            ...
                            dig.addModel("会员所有信息", url + "UserIndex/"+eId+"?userId=" + eId, 1000, 550, function () {
                                tb.initTable(1);
                            });
                        });
                    }
    ...
    sendEmail: function () {
        ...
        if (!vals) { dig.alertError("提示", "请选择要发送人的信息！"); return; }
        if (lsum > 1) { dig.alertError("提示", "能选择一项！"); return; }
        dig.addModel("添加/编辑管理员", url + "SendMail/?userId=" + vals, 850, 480, function () {
            tb.initTable(1);
        });
    },
    UserCustom: function(s) {
    }
};