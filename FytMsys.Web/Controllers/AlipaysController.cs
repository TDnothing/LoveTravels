            ...
            if (sPara.Count > 0)//判断是否有带返回参数
            {
                ...
                if (verifyResult)//验证成功
                {
                    ...
                    else if (Request.Form["trade_status"] == "TRADE_SUCCESS")
                    {
                        ...
                        //根据订单号判断支付类型
                        //LogHelper.WriteLog(typeof(AlipaysController), "订单号：" + out_trade_no + "<br />");
                        ...
                        if (header == "WD")
                        {
                            ...
                            if (oModel != null)
                            {
                                //LogHelper.WriteLog(typeof(AlipaysController), "支付状态：" + oModel.PayStatus + "<br />");
                                //LogHelper.WriteLog(typeof(AlipaysController), "支付金额：" + oModel.PayPrice + "<br />");
                                if (oModel.PayStatus == 1)
                                {
                                    return Content("success");
                                }
                                var total_fee = Request.Form["total_fee"];
                                //LogHelper.WriteLog(typeof(AlipaysController), "支付宝返回金额：" + total_fee + "<br />");
                                //var np = Math.Round(oModel.PayPrice, 0); //订单金额，转换成分，进行对比
                                //LogHelper.WriteLog(typeof(AlipaysController), "转换金额：" + np + "<br />");
                                //判断金额是否相等
                                if (total_fee == oModel.PayPrice.ToString())
                                {
                                    //LogHelper.WriteLog(typeof(AlipaysController), "金额匹配<br />");
                                ...
                                {
                                    LogHelper.WriteLog(typeof(AlipaysController), "金额不匹配<br />");
        ...
        public ActionResult ReturnUrl()
        {
            ...
            if (sPara.Count > 0)//判断是否有带返回参数
            {
                ...
                if (verifyResult)//验证成功
                {
                    ...
                    //LogHelper.WriteLog(typeof(AlipaysController), "交易状态：" + Request.QueryString["trade_status"] + "<br />");
                    ...
                    {
                        //LogHelper.WriteLog(typeof(AlipaysController), "支付状态：" + Request.QueryString["trade_status"] + "<br />");
