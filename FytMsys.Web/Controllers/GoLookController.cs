...
namespace FytMsys.Web.Controllers
{
    ...
    public class GoLookController : Controller
    {
        ...
        public ActionResult Publish()
        {
            ...
            var model = new lv_GoLook() { Flags = flag == "group" ? 1 : 0 ,Price = "0"};
        ...
        public ActionResult Detail(int id)
        {
            ...
            var commentlist = OperateContext<tb_Comment>.SetServer.GetList(
                PredicateBuilder.True<tb_Comment>().And(m => m.Go_LookId == id).And(m => m.Option == 1)
                    ...
                    if (joinnum < int.Parse(glo.Rsum))
                    {
                        ...
                        if (!IsExist)
                        {
                            ...
                            var model = new lv_GoLookOrder()
                            {
                                ...
                                PayStatus = 0,
                                PayName = info,
                                PayPrice =payType=="zfb" ? Convert.ToDecimal(glo.Price) : Convert.ToDecimal(glo.UsdPrice),
                                AddTime = DateTime.Now
                            };
