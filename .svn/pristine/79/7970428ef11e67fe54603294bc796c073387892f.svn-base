$(function () {
    ...
    $("#Modfiy").click(function () {
        ...
        dig.addModel("旅程故事信息", url + "Detail/" + vals, 1000, 550, function () {
            tb.initTable(1);
        });
    });
});

var url = "/FytAdmin/FytStory/", collSum = 6, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
                ...
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        $("#tables tr").dblclick(function () {
                            ...
                            dig.addModel("旅程故事信息", url + "Detail/" + eId, 1000, 550, function () {
                                tb.initTable(1);
                            });
                        });
                    }
    ...
    changetop: function (types) {
        ...
        var msg = "您确定修改选中项精华状态吗？";
        dig.confim("修改确认", msg, function () {
            $.post(url + "TopSet", { id: vals, types: types }, function (res) {
                if (res.Status == "y") tb.initTable(1);
                else dig.alertError("消息", res.Msg);
                $(":checkbox").attr("checked", false);
            }, "json");
