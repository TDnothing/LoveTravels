$(function () {
    ...
    $("#Modfiy").click(function () {
        ...
        window.location.href = url + "Modfiy/" + vals + "?ClassId=" + $("#classId").val();
    });
});

var url = "/FytAdmin/FytProject/", collSum = 5, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        var pageSize = $("#sel_page").val(), maxi = 0;
        $.ajax({
            type: "post",
            url: url + "UserCommentData",
            beforeSend: function () { tbody.html(tb.ajaxLoading()); },
            data: {
                pageSize: pageSize, pageIndex: pageIndex, key: escape($("#txtKey").val()), projectId: $("#projectId").val()
            },
...
function GetStauts(s) {
    var info = "";
    switch (s) {
        case 0: info = "未支付"; break;
        case 1: info = "已支付"; break;
        case 2: info = "支付失败"; break;
    }
    return info;
}