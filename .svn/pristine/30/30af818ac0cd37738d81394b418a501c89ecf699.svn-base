$(function () {
    ...
    $("#Modfiy").click(function () {
        ...
        if (!vals) { dig.alertError("提示", "请选择要查看那信息！"); return; }
        if (lsum > 1) { dig.alertError("提示", "信息只能选择一项！"); return; }
        var winH = $(window).height()-100;
        dig.addModel("留言详细", url + "Modfiy/" + vals, 700, winH, function () {
            //tb.initTable(1);
        });
    });
});

var url = "/FytAdmin/Message/", collSum = 6, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        ...
        $.ajax({
            ...
            data: {
                ...
                ClassId: $("#ClassId").val(), beginTime: $("#beginTime").val(), endTime: $("#endTime").val()
            },
...
function GetDetail(id) {
    var winH = $(window).height() - 100;
    dig.addModel("留言详细", url + "Modfiy/" + id, 700, winH, function () {
        //tb.initTable(1);
    });
}
