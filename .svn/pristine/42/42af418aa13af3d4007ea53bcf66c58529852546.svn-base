$(function () {
    ...
    $("#Insert").click(function () {
        window.location.href = url + "Modify?id=0";
    });
    $("#Modfiy").click(function () {
        ...
        dig.addModel("查看订单详细", url + "Modify/" + vals, 1000, winH - 200, function () {
            tb.initTable(1);
        });
    });
});
var url = "/FytAdmin/OrderMan/", collSum = 9, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        ...
        $.ajax({
            ...
            data: {
                ...
                endTime: $("#endTime").val(),
                key: escape($("#txtKey").val()),
                orderStatus: $("#audit").val()
            },
                ...
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        $("#tables tr").dblclick(function () {
                            ...
                            var winH = $(window).height();
                            dig.addModel("查看订单详细", url + "Modify/" + eId, 1000, winH - 200, function () {
                                tb.initTable(1);
                            });
                        });
                    }
