$(function () {
    ...
    $("#Modfiy").click(function () {
        ...
        if (lsum > 1) { dig.alertError("提示", "只能选择一条记录！"); return; }
        dig.addModel("消息日志详细", url + "LogDetail/" + vals, 850, 480, function () {
            tb.initTable(1);
        });
    });
});
...
var url = "/FytAdmin/Member/", collSum = 8, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        ...
        $.ajax({
            ...
            data: {
                pageSize: pageSize, pageIndex: pageIndex, key: escape($("#txtKey").val()), logType: escape($("#logType").val()),
                userId: $("#userId").val()
            },
                ...
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        $("#tables tr").dblclick(function () {
                            ...
                            dig.addModel("消息日志详细", url + "LogDetail/" + eId, 850, 480, function () {
                                tb.initTable(1);
                            });
                        });
                    }
    ...
    deletes: function (id) {
        ...
        dig.confim("删除确认", msg, function () {
            $.post(url + "LogDeleteBy", { id: vals }, function (res) {
