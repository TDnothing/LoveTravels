$(function () {
    ...
    $("#Modfiy").click(function () {
        ...
        if (!vals) { dig.alertError("提示", "请选择要查看的信息！"); return; }
        if (lsum > 1) { dig.alertError("提示", "查看信息只能选择一项！"); return; }
        dig.addModel("查看评论详细", url + "Modfiy/" + vals, 750, 480, function () {
            tb.initTable(1);
        });
    });
});

var url = "/FytAdmin/FytComment/", collSum = 9, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
    initTable: function (pageIndex) {
        ...
        $.ajax({
            ...
            data: {
                pageSize: pageSize, pageIndex: pageIndex, key: escape($("#txtKey").val()), status: $("#selStatus").val(),
                ...
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        $("#tables tr").dblclick(function () {
                            ...
                            dig.addModel("查看评论详细", url + "Modfiy/" + eId, 750, 480, function () {
                                tb.initTable(1);
                            });
                        });
                    }
    ...
    resStatus: function (s) {
        switch (parseInt(s)) {
        case 1:
            return "未审核";
            case 2:
                return '<span class="text-navy"><i class="fa fa-level-up"></i> 审核通过</span>';
            case 3:
                return '<span class="text-warning"><i class="fa fa-level-down"></i> 审核未通过</span>';
        default:
        }
        return s;
    },
    ModfiyStatus: function (t,id) {
        var vals = '';
        if (id == 0) {
            $('input[name="checkbox_name"]:checked').each(function () {
                vals += $(this).val() + ',';
        ...
        var msg = "审核状态将要更改，您确定吗？";
        dig.confim("审核确认", msg, function () {
            $.post(url + "ModfiyStatus", { id: vals, type: t }, function (res) {
                if (res.Status == "y") tb.initTable(1);
                else dig.alertError("消息", res.Msg);
                $(":checkbox").attr("checked", false);
            }, "json");
