...
namespace FytMsys.Common
{
    public class WaterMark
    {
        ...
        {
            if(!File.Exists(UtilsHelper.GetMapPath(imgPath)))
                return;
            byte[] _ImageBytes = File.ReadAllBytes(UtilsHelper.GetMapPath(imgPath));
            Image img = Image.FromStream(new System.IO.MemoryStream(_ImageBytes));
            filename = UtilsHelper.GetMapPath(filename);

            if (watermarkFilename.StartsWith("/") == false)
                watermarkFilename = "/" + watermarkFilename;
            watermarkFilename = UtilsHelper.GetMapPath(watermarkFilename);
