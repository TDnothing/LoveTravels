$(function () {
    ...
    $("#Modfiy").click(function () {
        ...
        dig.addModel("参与人员信息", url + "UserIndex/" + vals + "?id=" + vals, 1000, 550, function () {
            tb.initTable(1);
        });
    });
});

var url = "/FytAdmin/FytProject/", collSum = 11, tbody = $("#tables>tbody"), tdata = $("#tlist"), tb = {
                ...
                if (res.Status == "y") {
                    if (res.Data == "" || res.Data == null) {
                        ...
                        $("#tables tr").dblclick(function () {
                            ...
                            dig.addModel("用户评论信息", url + "UserCommentIndex/" + eId + "?id=" + eId, 1000, 550, function () {
                                tb.initTable(1);
                            });
                        });
                    }
    ...
    changespecial: function (id) {
        ...
        var msg = "您确定修改选中项特色状态吗？";
        dig.confim("修改确认", msg, function () {
            $.post(url + "ChangeIsSpecial", { id: vals }, function (res) {
                if (res.Status == "y") tb.initTable(1);
                else dig.alertError("消息", res.Msg);
                $(":checkbox").attr("checked", false);
            }, "json");
    ...
    changeaudit: function (id, status) {
        var vals = '';
        if (id == 0) {
            $('input[name="checkbox_name"]:checked').each(function () {
                vals += $(this).val() + ',';
        ...
        var msg = "您确定修改选中项审核状态吗？";
        dig.confim("修改确认", msg, function () {
            $.post(url + "ChangeAudit", { id: vals, status: status }, function (res) {
                if (res.Status == "y") tb.initTable(1);
                else dig.alertError("消息", res.Msg);
                $(":checkbox").attr("checked", false);
            }, "json");
    ...
    changerecommend: function (id) {
        ...
        var msg = "您确定修改选中项推荐状态吗？";
        dig.confim("修改确认", msg, function () {
            $.post(url + "ChangeIsRecommend", { id: vals }, function (res) {
                if (res.Status == "y") tb.initTable(1);
                else dig.alertError("消息", res.Msg);
                $(":checkbox").attr("checked", false);
            }, "json");
